...
online$pos_XY = paste(online$posX, online$posY, sep = "-")


grid_comb = with(offline, by(offline, list(pos_XY, rounded_orient, mac), function(x) x))

grouped_signal = lapply(grid_comb, function(x) {
  ans = x[1, ]
  ans$avgSignal = mean(x$signal)
})



offlineSummary = do.call("rbind", grouped_signal)

vars = c("pos_XY", "posX","posY", "orientation", "rounded_orient")

on_group = with(online,
...
onlineSummary = do.call("rbind", on_group)
offlineSummary = do.call('rbind', off_group)
